...
class Api::V1::LabwaresController < Api::V1::ApplicationController
  ...
  def set_labware
    @labware = Labware.includes(receptacles: :location, labware_type: {layout: :locations}).find_by(uuid: params[:id])
  end

  # Only allow a trusted parameter "white list" through.
  def labware_params
    params = (labware_json_params.dig(:data, :attributes) or {}).merge(uuid: labware_json_params.dig(:data, :id)).delete_if { |k, v| v.nil? }

    labware_type = @labware ? @labware.labware_type : nil
    if (labware_type_params = labware_json_params.dig(:data, :relationships, :labware_type, :data, :attributes))
    ...
    if (metadata_data = labware_json_params.dig(:data, :relationships, :metadata, :data))
    ...
    if (receptacles_data = labware_json_params.dig(:data, :relationships, :receptacles, :data))
